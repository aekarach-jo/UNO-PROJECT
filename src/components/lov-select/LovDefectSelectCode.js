...
function mapOptions(lovList, lovLabel, lovValue, typeList, typeMessage = 'W') {
  ...
  const optionFilter = lovList.filter((item) =>
    typeFilter.some((item2) => {
      return item2.id === item.mainLinkId;
    })
  );
  ...
  useEffect(() => {
    ...
    if (value !== '') {
      options.forEach((dataI) => {
        if (value[0]?.detail === undefined) {
          for (let i = 0; i < value.length; i += 1) {
            if (value[i] === dataI.detail?.code) {
              findOption.push(dataI);
            }
          }
        } else {
          value?.forEach((dataJ) => {
            if (dataJ.detail?.code === dataI.detail?.code) {
              findOption.push(dataI);
            }
          });
        }
...
export default LovDefectSelectCode;
