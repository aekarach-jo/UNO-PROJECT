...
function mapOptions(data, lovLabel, lovValue) {
  return (data || []).map((lov) => ({
    label: lov[lovLabel],
    value: lov[lovValue],
    detail: lov,
...
const LovPatchingSelect = ({
  ...
  const { usePacthingLov } = useProductLovData();
  const { data, isFetching, refetch } = usePacthingLov({ isGroup: true });
...
export default LovPatchingSelect;
